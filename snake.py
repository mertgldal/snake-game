from turtle import Turtle
import random

STARTING_POSITIONS = [(0, 0), (-20, 0), (-40, 0)]
MOVE_DISTANCE = 20
UP = 90
DOWN = 270
LEFT = 180
RIGHT = 0


class Snake:
    def __init__(self, ):
        self.segments = []
        self.colors = ["white", "light sky blue", "aquamarine",
                       "green yellow", "gold", "wheat", "orange",
                       "crimson", "salmon", "lavender", "dark orange",
                       "hot pink", "deep pink", "lavender blush",
                       "medium purple", "medium orchid", "peru",
                       "medium spring green", "honeydew", "violet",
                       "spring green", "pale turquoise", "deep pink",
                       "dodger blue", "dark slate blue", "forest green"]
        self.create_snake()
        self.head = self.segments[0]

    def create_snake(self):
        for position in STARTING_POSITIONS:
            self.add_segment(position)

    def add_segment(self, position):
        rand_color = random.choice(self.colors)
        new_segment = Turtle(shape="circle")
        new_segment.color(rand_color)
        new_segment.penup()
        new_segment.goto(position)
        self.segments.append(new_segment)
        self.colors.remove(rand_color)

    def extend(self):
        self.add_segment(self.segments[-1].position())

    def move(self):
        # If we try to run program like this, program occur an error like TypeError: range() takes no keyword arguments.
        # The reason is that, range function legacy from C language.
        # for seg_num in range(start=2, stop=0, step=-1):
        for seg_num in range(len(self.segments) - 1, 0, -1):
            new_x = self.segments[seg_num - 1].xcor()
            new_y = self.segments[seg_num - 1].ycor()
            self.segments[seg_num].goto(new_x, new_y)
        self.head.forward(MOVE_DISTANCE)

    def up(self):
        if self.head.heading() != DOWN:
            self.head.seth(UP)

    def down(self):
        if self.head.heading() != UP:
            self.head.seth(DOWN)

    def left(self):
        if self.head.heading() != RIGHT:
            self.head.seth(LEFT)

    def right(self):
        if self.head.heading() != LEFT:
            self.head.seth(RIGHT)

    def reset_colors(self):
        self.colors = ["white", "light sky blue", "aquamarine",
                       "green yellow", "gold", "wheat", "orange",
                       "crimson", "salmon", "lavender", "dark orange",
                       "hot pink", "deep pink", "lavender blush",
                       "medium purple", "medium orchid", "peru",
                       "medium spring green", "honeydew", "violet",
                       "spring green", "pale turquoise", "deep pink",
                       "dodger blue", "dark slate blue", "forest green"]
